...
class BERTJointProcess(object):
    ...
    def save_sent_result(self, all_asr_list, all_pred_list, all_label_list):
        ...
        FileUtil.write_raw_data(save_sent_list, "result/joint_asr_label_not_same.txt")
    ...
    def output_sent_attention(self): 
        """
        打印单条句子的attention
        @param:
        @return:
        """
        text = "宣 x u a n 新 区 建 色 s e"
        token_input_dict = self.bert_tokenizer(text, truncation=True, padding="max_length", max_length=13, return_tensors="pt")
        token_input_dict = {k: v.to(self.args.device) for k, v in token_input_dict.items()}
        correct_seq_output, attentions = self.bert_model((token_input_dict["input_ids"], token_input_dict["attention_mask"], token_input_dict["token_type_ids"]))
        attentions = torch.stack(attentions, dim=0)
        attentions = attentions.transpose(0, 1)
        for layer_index in range(12):
            for offset in range(13):
                token_list = ["[CLS]"] + text.split() + ["[SEP]"]
                for layer_index in range(12):
                    tmp_list = attentions[0][layer_index][-1][offset].cpu().detach().numpy().tolist()
                    index_dict = {i:att for i, att in enumerate(tmp_list)}
                    index_att_sort_list = sorted(index_dict.items(), key=lambda x: x[1], reverse=True)
                    word_att_list = [(token_list[index], att) for index, att in index_att_sort_list]
                    print(layer_index, text, "############", offset, token_list[offset])
                    print(word_att_list, layer_index, tmp_list, sum(tmp_list)) 
                print("\n")

    def output_bert_token_embedding(self):
        """
        ...
        bert_model = BertModel.from_pretrained("/ssd1/users/fangzheng/data/mt_error/pretrain_model/BERT-wwm-ext")
        bert_model.to(self.args.device)
        data_obj_list = FileUtil.read_json_data("/ssd1/users/fangzheng/data/mt_error/correct_format/tmp_train.txt")
        LogUtil.logger.info("模型加载完毕!")

        all_embed_list = []
        for data_obj in data_obj_list:
            # 只有包含错误的句子才会有拼音输入
            if len(data_obj["errors"]) > 0:
                correct_input = data_obj["errors"][0]["correct_input"]
                token_input_dict = self.bert_tokenizer(correct_input, truncation=True, padding="max_length", max_length=48, return_tensors="pt")
                correct_inputs = self.bert_tokenizer.batch_decode(token_input_dict["input_ids"].numpy())
                
                token_input_dict = {k: v.to(self.args.device) for k, v in token_input_dict.items()}
                correct_seq_output, _ = bert_model(input_ids=token_input_dict["input_ids"], token_type_ids=token_input_dict["token_type_ids"], attention_mask=token_input_dict["attention_mask"], return_dict=False)
                
                spcial_token_set = set(["[SEP]", "[PAD]", "[CLS]"])
                for seq_embed, sent_input_text in zip(correct_seq_output, correct_inputs):
                    for token, token_embed in zip(sent_input_text.split(), seq_embed):
                        if token in spcial_token_set:
                            continue
                        ...
                        if re.search('[a-z]', token):
                            label = 1
                        all_embed_list.append([token, label, token_embed.detach().cpu().numpy().tolist()])
        
        LogUtil.logger.info("写入token embedding")
        FileUtil.write_json_in_append(all_embed_list, "/ssd1/users/fangzheng/data/mt_error/correct_format/source_bert_token_embed.txt")
