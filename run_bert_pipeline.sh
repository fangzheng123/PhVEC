...
# 微调模型存储路径
FINETUNE_MODEL_DIR=$ROOT_DIR/model/bert_pipeline_model
FINETUNE_MODEL_PATH=$FINETUNE_MODEL_DIR/bert_detect.pk
...
nohup python -u run_model/run_bert_pipeline_correction.py \
  --do_train \
  --pretrain_model_path=$PRE_TRAINED_MODEL_DIR \
  --output_dir=$FINE_TUNING_MODEL_DIR \
  --model_save_path=$FINETUNE_MODEL_PATH \
  --detect_model_path=$FINETUNE_MODEL_DIR/bert_detect.pk \
  --correct_model_path=$FINETUNE_MODEL_DIR/bert_correct.pk \
  --logging_dir=$LOG_DIR \
